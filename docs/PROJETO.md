...
API em **FastAPI** que **coleta** notícias públicas de uma fonte configurada (hoje: **G1 Tecnologia**), **persiste** em **PostgreSQL** com deduplicação por URL e **expõe** listagem paginada com documentação OpenAPI em `/docs`.
...
app/
  ...
  services.py      # Coleta, contagem e listagem (regras de negócio)
  ...
  core/config.py   # Configuração (ex.: DATABASE_URL)
```
...
Convenção adotada: recurso em plural **`/news`**; verbo na **ação HTTP** (`POST` coleta, `GET` consulta).
